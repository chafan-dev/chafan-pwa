                    ...
                    <v-row>
                        <v-col class="d-flex" align-self="center">
                            <v-dialog max-width="300" v-model="showCancelUpvoteDialog">
                                ...
                                <v-card-actions>
                                    <v-spacer />
                                    <v-btn small @click="showCancelUpvoteDialog = false">{{$t('No')}}</v-btn>
                                    <v-btn small color="warning" @click="cancelUpvote" :disabled='cancelUpvoteIntermediate'>{{$t('Yes')}}</v-btn>
                                </v-card-actions>
                                </v-card>
                            </v-dialog>
                            <v-btn @click="showCancelUpvoteDialog = true" small
                                    class="slim-btn ma-1" color="primary lighten-2" v-if="upvotes && upvotes.upvoted">
                                {{$t('已赞')}} ({{ upvotes.count }})
                            </v-btn>
                            <v-btn small class="slim-btn ma-1" color="primary"
                                    @click="upvote" v-else :disabled="currentUserIsAuthor || upvoteIntermediate">
                                {{$t('赞')}} ({{ upvotes.count }})
                            </v-btn>

                            <v-btn small class="slim-btn ma-1" @click="toggleShowComments">

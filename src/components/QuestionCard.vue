                ...
                <v-row class="ma-2" v-if="myToken">
                    ...
                    <v-dialog max-width="500" v-model="showInviteToAnswerDialog">
                        ...
                        <v-card-text>
                            ...
                            <v-slider :label="$t('Reward')" v-model="inviteToAnswerRewardCoinAmount"
                                      class="mt-2"  :tick-labels="[0, 1, 2, 3, 4]"
                                      step="1" min="0" max="4" thumb-label ticks="always" />
                            <div class="text-caption grey--text">
                                {{$t('Reward is the amount of coins rewarded to the invited user when the user published an answer.')}}
                                {{$t("The coins will be first deducted from your account, and you can reclaim it after the reward expired (if the user doens't post an answer in one week).")}}
        ...
        await dispatchCaptureApiError(this.$store, async () => {
            if (this.invitedUserId !== null) {
                const response = await api.inviteAnswer(
                    this.$store.state.main.token, this.question.uuid, this.invitedUserId);

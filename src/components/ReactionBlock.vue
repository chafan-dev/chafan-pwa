<template>
    <div>
        <v-btn-toggle dense v-if="reactions" multiple>
            <v-btn small v-for="(count, reaction) in reactions.counters" :key="reaction"
                    @click="updateReaction(reaction, reactions.my_reactions.includes(reaction) ? 'cancel' : 'add')"
        ...
        <v-menu offset-x v-if="loggedIn">
            ...
            <v-list>
                <v-list-item v-for="(reaction, index) in myReactionChoices" :key="index">
                    <v-list-item-content style="cursor: pointer" @click="updateReaction(reaction, 'add')" class="pr-1">
                        {{reaction}}
                    </v-list-item-content>
                </v-list-item>
            </v-list>
        </v-menu>
        <v-progress-circular class="ml-2" color="primary" indeterminate size="20" v-if="reactionIntermediate"/>
    </div>
</template>
...
export default class ReactionBlock extends Vue {
    ...
    private async mounted() {
        this.reactions = (await api2.getReactions(this.$store.state.main.token, this.objectId, this.objectType)).data;
    ...
    private async updateReaction(reaction: '👍' | '👎' | '👀' | '❤️' | '😂' | '🙏', action: 'add' | 'cancel') {
        if (!this.loggedIn) {
            return;
        }

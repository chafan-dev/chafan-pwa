<template>
    <div class="ma-3 pa-3">
        ...
        <v-card-text v-if="topicSubscription">
            ...
            <div class="mt-2">
                <v-btn @click="cancelSubscription" :disabled="cancelSubscriptionIntermediate" v-if="topicSubscription.subscribed_by_me">
...
export default class TopicCard extends Vue {
    ...
    public async mounted() {
        ...
        await dispatchCaptureApiError(this.$store, async () => {
            const response3 = await apiMe.getTopicSubscription(this.$store.state.main.token, this.topic.uuid);
            this.topicSubscription = response3.data;
        });
        if (this.topic.parent_topic_uuid) {
            this.parentTopic = (await apiTopic.getTopic(this.$store.state.main.token, this.topic.parent_topic_uuid)).data;
        }
    }

...
import { ICoinPayment, IComment, IMsg, INotification,
  IQuestionPreview, IReaction, IReactions, IUserInvite } from './interfaces';
...
export const api2 = {
  ...
  async getSubscribedQuestions(token: string) {
    return axios.get<IQuestionPreview[]>(`${apiUrl}/api/v1/me/question-subscriptions/`, authHeaders(token));
  },
  ...
  async getChildComments(token: string, commentId: string) {
    return axios.get<IComment[]>(`${apiUrl}/api/v1/comments/${commentId}/child-comments/`, authHeaders(token));
  },
  ...
  async getReactions(token: string, objectId: string, objectType: 'question' | 'answer' | 'comment' | 'article') {
    return axios.get<IReactions>(`${apiUrl}/api/v1/reactions/${objectType}/${objectId}`, authHeaders(token));
  },

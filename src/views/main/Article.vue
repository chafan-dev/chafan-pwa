        ...
        <v-col :class="{'col-8': $vuetify.breakpoint.mdAndUp,
            ...
            <div v-if="article">
                <div v-if="article" class="ma-4">
                    ...
                    <div class="ma-2">
                        <v-chip small class="ml-2" color="warning" v-if="article && !article.is_published">{{$t('此为初稿仅自己可见')}}</v-chip>
                        <v-chip small class="ml-2" color="info" v-else-if="showHasDraftBadge">{{$t('编辑器中有未发表的草稿')}}</v-chip>
                    ...
                    <div fluid>
                        <v-row>
                            <v-col>
                                <template v-if="token">
                                    <v-dialog max-width="300" v-model="showCancelUpvoteDialog">
                                        <v-card>
                                            <v-card-title primary-title>
                                                <div class="headline primary--text">{{$t('确定收回点赞？')}}</div>
                                            </v-card-title>
                                        <v-card-actions>
                                            <v-spacer />
                                    ...
                                    <v-btn @click="showCancelUpvoteDialog = true"
                                        :small="!$vuetify.breakpoint.mdAndUp" class="slim-btn ma-1" color="primary lighten-2" v-if="upvotes && upvotes.upvoted">
                                        {{$t('已赞')}} ({{ upvotes.count }})
                                    </v-btn>
                                    <v-btn :small="!$vuetify.breakpoint.mdAndUp" class="slim-btn ma-1" color="primary" @click="upvote"
                                            v-else :disabled="currentUserIsAuthor || upvoteIntermediate">
                                        {{$t('赞')}} ({{ upvotes.count }})
                                    </v-btn>
                                </template>

                                <v-btn :small="!$vuetify.breakpoint.mdAndUp" class="slim-btn ma-1"
                                ...
                                <v-menu offset-y v-if="currentUserIsAuthor">
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-btn v-bind="attrs" v-on="on" class="slim-btn ma-1"
                                            :small="!$vuetify.breakpoint.mdAndUp" >{{$t('设置')}}</v-btn>
                                    </template>
                                    <v-list>
                                ...
                                <v-dialog max-width="300" v-model="confirmDeleteDialog">
                                    <v-card>
                                        <v-card-title primary-title>
                                            <div class="headline primary--text">{{$t('确定永久删除文章及其所有历史版本？')}}</div>
                                        </v-card-title>
                                    <v-card-actions>
                                        <v-spacer />
                                        <v-btn @click="confirmDeleteDialog = false">{{$t('No')}}</v-btn>
                                        <v-btn color="warning" @click="deleteArticle" :disabled='deleteArticleIntermediate'>{{$t('Yes')}}</v-btn>
                                    </v-card-actions>
                                    </v-card>
                                </v-dialog>
                            ...
                            <v-col md="auto" v-if="$vuetify.breakpoint.mdAndUp && article.view_times">
                                <span class="text-caption grey--text ma-2">{{$t('已被阅读n次', { times: article.view_times })}}</span>
                            </v-col>
                        </v-row>
...
export default class Article extends Vue {
    ...
    private async unbookmark() {
        ...
        await dispatchCaptureApiError(this.$store, async () => {
            if (this.article) {
                this.userBookmark = (await apiArticle.unbookmarkArticle(this.token, this.article.uuid)).data;
    ...
    private updateStateWithLoadedArticle(article: IArticle) {
        if (!this.$route.query.title) {
            this.$router.replace({ query: { ...this.$route.query, title: article.title }});
        }

                ...
                <v-tabs v-model="currentTabItem" show-arrows
                    ...
                    <v-tab-item value="profile">
                        <template v-if="userPublic">
                            <div class="my-3" v-if="userPublic.homepage_url || userPublic.github_username || userPublic.twitter_username || userPublic.linkedin_url">
                                ...
                                <a class="text-decoration-none" v-if="userPublic.homepage_url" :href="userPublic.homepage_url" target="_blank">
                                    <WebIcon /> {{$t('个人主页')}}
                                </a>
                                <a class="text-decoration-none" v-if="userPublic.github_username"
                                    :href="canonicalURLfromUsername(userPublic.github_username, 'github.com')" target="_blank">
                                    <GithubIcon /> Github
                                </a>
                                <a class="text-decoration-none" v-if="userPublic.twitter_username"
                                    :href="canonicalURLfromUsername(userPublic.twitter_username, 'twitter.com')" target="_blank">
                                    <TwitterIcon /> Twitter
                                </a>
                                <a class="text-decoration-none" v-if="userPublic.linkedin_url" :href="userPublic.linkedin_url" target="_blank">
                        ...
                        <template v-if="userPublic">
                            <div class="my-3" v-if="userPublic.residency_topics.length > 0">
                                <div class="subheading secondary--text text--lighten-3">{{$t('居住过的地方')}}</div>
                                <div>
                                    <v-chip-group :column="!$vuetify.breakpoint.mobile">
                                        <v-chip :to="'/topics/' + topic.uuid" v-for="topic in userPublic.residency_topics" :key="topic.uuid">{{ topic.name }}</v-chip>
                                    </v-chip-group>
                                </div>
                            </div>
                        </template>
                        ...
                        <template v-if="userPublic">
                            <div class="my-3" v-if="userPublic.profession_topic">
                                <div class="subheading secondary--text text--lighten-3">{{$t('所在行业')}}</div>
                                <div class="title primary--text text--darken-2">
                                    <v-chip :to="'/topics/' + userPublic.profession_topic.uuid">{{ userPublic.profession_topic.name }}</v-chip>
                                </div>
                            </div>
                        </template>
                        <v-skeleton-loader type="text" v-else />
                        ...
                        <template v-if="eduExps">
                            <div class="my-3" v-if="eduExps.length > 0">
                                <div class="subheading secondary--text text--lighten-3">{{$t('教育经历')}}</div>
                                <div class="title primary--text text--darken-2" v-for="(eduExp, index) in eduExps" :key="index">
                        ...
                        <template class="my-3" v-if="workExps">
                            <div class="my-3" v-if="workExps.length > 0">
                                <div class="subheading secondary--text text--lighten-3">{{$t('工作经历')}}</div>
                                <div class="title primary--text text--darken-2" v-for="(workExp, index) in workExps" :key="index">
                                    <a class="text-decoration-none" :href="'/topics/' + workExp.position_topic.uuid">
                    ...
                    <v-tab-item value="answers">
                        <template v-if="authoredAnswers !== null">
                            ...
                            <Answer v-for="answer in authoredAnswers" :key="answer.uuid"
                                    class="my-4" :answerPreview="answer" />
                    ...
                    <v-tab-item value="questions">
                        <template v-if="askedQuestions !== null">
                            ...
                            <QuestionPreview :questionPreview="question" class="my-4"
                    ...
                    <v-tab-item value="articles">
                        <template v-if="articles !== null">
                            ...
                            <ArticlePreview :articlePreview="article" class="my-4"
                    ...
                    <v-tab-item value="submissions">
                        <template v-if="submissions !== null">
                            ...
                            <SubmissionCard :submission="submission" class="my-4"
...
import { IAnswerPreview, IArticlePreview, IQuestionPreview, ISite, ISubmission, IUserEducationExperience,
IUserPreview, IUserPublic, IUserPublicForVisitor, IUserWorkExperience } from '@/interfaces';
...
@Component({
    components: {
        QuestionPreview, Answer, UserCard, UserLink, SiteBtn,
        TwitterIcon, WebIcon, GithubIcon, UserGrid,
        LinkedinIcon, ArticlePreview, SubmissionCard,
    },
})
export default class User extends Vue {
    ...
    public async mounted() {
        await dispatchCaptureApiError(this.$store, async () => {
            if (this.loggedIn) {
                ...
                await Promise.all(this.tabItems.map(async (item) => {
    ...
    private async renderProfileTab(userPublic: IUserPublic) {
        this.eduExps = (await apiPeople.getUserEducationExperiences(
                    this.token, userPublic.uuid)).data;
        this.workExps = (await apiPeople.getUserWorkExperiences(
            this.token, userPublic.uuid)).data;
        const responses = await Promise.all(userPublic.profiles.map((p) => api.getSite(this.token, p.site.subdomain)));

                ...
                <div>
                    <v-card v-for="field in form.form_fields" :key="field.unique_name" class="ma-2 pa-4">
                        ...
                        <div v-if="field.field_type.field_type_name === 'text_field'">
                            <v-textarea :label="$t('Content')" v-model="responseFields.get(field.unique_name).field_content.text" />
                        </div>
                        <div v-else-if="field.field_type.field_type_name === 'single_choice_field'">
                            <v-radio-group v-model="responseFields.get(field.unique_name).field_content.selected_choice">
                                <v-radio v-for="choice in field.field_type.choices" :key="choice" :label="choice" :value="choice" />
                            </v-radio-group>
                        </div>
                        <div v-else-if="field.field_type.field_type_name === 'multiple_choices_field'">
                            <v-select :items="field.field_type.choices" multiple chips
...
export default class Form extends Vue {
    ...
    private async submitResponse() {
        ...
        this.formResponse = (await apiForm.submitFormRespnse(
            this.$store.state.main.token, this.formResponseCreate!)).data;

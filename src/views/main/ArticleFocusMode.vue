...
export default class ArticleFocusMode extends Vue {
    ...
    public async mounted() {
        if (this.articleId) {
            const article = (await apiArticle.getArticle(this.$store.state.main.token, this.articleId)).data;
            const articleDraft = (await apiArticle.getArticleDraft(this.$store.state.main.token, article.uuid)).data;
    ...
    public async newEditHandler(payload: {edit: IRichEditorState, isAutosaved: boolean,
                                          articleId?: string, writingSessionUUID: string,
        ...
        await dispatchCaptureApiError(this.$store, async () => {
            ...
            if (this.articleColumnId !== null && !this.articleId && !payload.articleId) {
                ...
                const article = await newArticleHandler(
                    this, payload.edit, payload.writingSessionUUID, payload.isAutosaved, this.articleColumnId);
                if (article) {
            ...
            } else if (this.articleColumnId !== null && (this.articleId || payload.articleId) && payload.edit.title) {

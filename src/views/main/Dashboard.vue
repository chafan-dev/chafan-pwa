        ...
        <v-tabs :vertical='$vuetify.breakpoint.mdAndUp'>
              ...
              <v-card class="ma-2 pa-2" :loading="answerBookmarksIntermediate">
                ...
                <v-card-text>
                  <v-switch :label="$t('发送邮件提醒未读通知')" @change="onChangeEmailNotifications"
                            v-model="enableEmailNotifications" :disabled="changingMySettings" />
                  ...
                  <v-dialog v-model="showExportDialog" max-width="500px">
                    <v-card>
                      ...
                      <v-card-text>
                        {{ $t('Chafan 支持用户的数据所有权和导出自由，所以你随时可以导出你拥有的数据和创作内容。目前自动导出尚未实现，请直接联系 takeout@cha.fan，我们将在一周内将你的数据快照发送到注册用的邮箱。') }}
                      </v-card-text>
                    </v-card>
                  </v-dialog>
                </v-card-text>
            ...
            <v-tab-item>
              ...
              <v-card class="ma-2 pa-2" :loading="draftsIntermediate">
                ...
                <v-card-text>
                  <Answer v-for="answer in myAnswerDrafts" :key="answer.uuid" class="mb-2"
              ...
              <v-card class="ma-2 pa-2" :loading="draftsIntermediate">
                ...
                <v-card-text>
                  <ArticlePreview v-for="article in myArticleDrafts" :key="article.uuid" :articlePreview="article" class="mb-2" />
                </v-card-text>
              </v-card>
            </v-tab-item>
            ...
            <v-tab-item>
              ...
              <v-card class="ma-2 pa-2" :loading="articleColumnsIntermediate">
                ...
                <v-dialog v-model="dialogNewArticleColumn" max-width="500px">
                  <v-card>
                    <v-card-title><span class="headline">{{$t('创建新专栏')}}</span></v-card-title>
                    <v-card-text>
                ...
                <ul>
                    <li v-for="articleColumn in myArticleColumns" :key="articleColumn.uuid">
                        <a class="text-decoration-none" :href="`/article-columns/${articleColumn.uuid}`">{{ articleColumn.name }}</a>
                    </li>
                </ul>
              </v-card>
            </v-tab-item>
            ...
            <v-tab-item>
              ...
              <v-card class="ma-2 pa-2" :loading="channelsIntermediate">
                ...
                <div>
                  <v-tabs :vertical='$vuetify.breakpoint.mdAndUp'>
                    <v-tabs-slider />
                    <v-tab v-for="channel in myChannels" :key="channel.code" style="justify-content: left">
                        <UserLink :userPreview="(channel.private_with_user && channel.private_with_user.uuid !== userProfile.uuid) ? channel.private_with_user : channel.admin"
                                  :showAvatar="true" :clickable="false" />
                    </v-tab>
            ...
            <v-tab-item>
              ...
              <v-card class="ma-2 pa-2" :loading="answerBookmarksIntermediate">
                ...
                <Answer :answerPreview="answer" v-for="answer in answerBookmarks" :key="answer.uuid" class="ma-2" />
              </v-card>
            </v-tab-item>
            ...
            <v-tab-item>
              <div class="d-flex ma-2">
                <div>
                  ...
                  <span class="title primary--text text--darken-2" v-if="userProfile">{{userProfile.remaining_coins}}</span>
              ...
              >
                ...
                <template v-slot:item.expired_at="{ item }">
                  <span v-if="item.expired_at">{{ $dayjs.utc(item.expired_at).local().fromNow() }}</span>
                </template>
                <template v-slot:item.claimed_at="{ item }">
                  <span v-if="item.claimed_at">{{ $dayjs.utc(item.claimed_at).local().fromNow() }}</span>
                </template>
                <template v-slot:item.refunded_at="{ item }">
                  <span v-if="item.refunded_at">{{ $dayjs.utc(item.refunded_at).local().fromNow() }}</span>
                </template>
                ...
                <template v-slot:item.type="{ item }">
                  {{ item.giver.uuid === userProfile.uuid ?
                        ...
                        (item.receiver.uuid === userProfile.uuid?
                              $t('Receive') + ' ← ':
                              $t('Unknown')) }}
                  <UserLink :userPreview="item.giver.uuid === userProfile.uuid ? item.receiver : item.giver" />
                </template>
                <template v-slot:item.condition="{ item }">
                  <span v-if="item.condition.content.condition_type === 'answered_question'">
                    {{$t('回答问题')}}: <a class="text-decoration-none" :href="'/questions/' + item.condition.content.question_uuid">{{$t('Link')}}</a>
                  </span>
                </template>
                ...
                <template v-slot:item.type="{ item }">
                  {{ item.payer.uuid === userProfile.uuid ?
                        ...
                        (item.payee.uuid === userProfile.uuid?
                              $t('Income') + ' ← ':
                              $t('Unknown')) }}
                  <UserLink :userPreview="item.payer.uuid === userProfile.uuid ? item.payee : item.payer"></UserLink>
                </template>
...
import { IAnswerPreview, IArticleColumn, IArticleColumnCreate, IArticlePreview,
IChannel, ICoinPayment, IQuestionPreview, IReward } from '@/interfaces';
import { api } from '@/api';
...
@Component({
    components: {
      QuestionLink, Answer, SiteBtn, ArticlePreview,
      UserLink, Event, InfoIcon, NewInviteLinkBtn, ChatWindow,
    },
})
export default class Dashboard extends Vue {
  ...
  private selectedEditorMode: 'markdown_realtime_rendering' | 'wysiwyg' | 'markdown_splitview' = 'wysiwyg';
  ...
  private async mounted() {
    await dispatchCaptureApiError(this.$store, async () => {
      if (this.userProfile) {
        ...
        this.subscribedQuestions = (await api2.getSubscribedQuestions(this.$store.state.main.token)).data;
  ...
  private async commitNewArticleColumn() {
    ...
    await dispatchCaptureApiError(this.$store, async () => {
      ...
      const response = await apiArticle.createArticleColumn(this.$store.state.main.token, this.newArticleColumn);
      this.$router.push(`/article-columns/${response.data.uuid}`);
    });
  }

  private claimable(reward: IReward) {
    if (this.$dayjs.utc(reward.expired_at).isBefore(this.$dayjs.utc()) || reward.claimed_at || reward.refunded_at) {
      return false;
    }
    if (this.userProfile) {
  ...
  private async onChangeEmailNotifications() {
    await dispatchCaptureApiError(this.$store, async () => {
      ...
      const newProfile = (await apiMe.updateMe(this.$store.state.main.token, {

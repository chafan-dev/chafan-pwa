...
import { IChannel, ISite, IUserFollows, IUserProfile, IUserQuestionSubscription, IUserSiteProfile,
    IUserTopicSubscription, IUserUpdateMe, IQuestionPreview,
    IUserUpdateLoginPhoneNumber, IUserUpdateSecondaryEmails, IUserUpdatePrimaryEmail } from '../interfaces';
...
export const apiMe = {
    ...
    async updateSecondaryEmail(token: string, data: IUserUpdateSecondaryEmails) {
        return axios.put<IUserProfile>(`${apiUrl}/api/v1/me/secondary-emails`, data, authHeaders(token));
    },
    ...
    async followUser(token: string, userUUID: string) {
        return axios.post<IUserFollows>(`${apiUrl}/api/v1/me/follows/${userUUID}`, null, authHeaders(token));
    },
    async cancelFollowUser(token: string, userUUID: string) {
        return axios.delete<IUserFollows>(`${apiUrl}/api/v1/me/follows/${userUUID}`, authHeaders(token));
    },
    async getQuestionSubscription(token: string, questionUUID: string) {
        return axios.get<IUserQuestionSubscription>(
            `${apiUrl}/api/v1/me/question-subscriptions/${questionUUID}`, authHeaders(token));
    },
    ...
    async subscribeQuestion(token: string, questionUUID: string) {
        return axios.post<IUserQuestionSubscription>(
            `${apiUrl}/api/v1/me/question-subscriptions/${questionUUID}`, null, authHeaders(token));
    },
    async getTopicSubscription(token: string, topicUUID: string) {
        return axios.get<IUserTopicSubscription>(
            `${apiUrl}/api/v1/me/topic-subscriptions/${topicUUID}`, authHeaders(token));
    },
    ...
    async subscribeTopic(token: string, topicUUID: string) {
        return axios.post<IUserTopicSubscription>(
            `${apiUrl}/api/v1/me/topic-subscriptions/${topicUUID}`, null, authHeaders(token));
    },
    ...
    async getPendingQuestions(token: string) {
        return axios.get<IQuestionPreview[]>(`${apiUrl}/api/v1/me/pending-questions/`, authHeaders(token));
    },
};
